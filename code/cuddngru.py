...
from keras.layers import Dense, Input, LSTM, Embedding, Dropout, Activation,GRU,Conv1D,GlobalMaxPool1D,MaxPooling1D,CuDNNGRU,TimeDistributed, Lambda, multiply,concatenate,CuDNNLSTM
...
x = CuDNNGRU(400, return_sequences=True)(embedded_sequences)
x = TimeDistributed(Dense(400, activation='tanh'))(x)
x = Lambda(lambda x: K.max(x, axis=1), output_shape=(400, ))(x)
merged = Dense(400,activation='relu')(x)
...
STAMP = 'cudnngru_glove_vectors_%.2f_%.2f' % (rate_drop_lstm, rate_drop_dense)
print(STAMP)

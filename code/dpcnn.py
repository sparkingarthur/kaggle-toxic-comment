...
convlayer = Conv1D(filters=filternumber, kernel_size=5, activation='relu', padding='same')(embedded_sequences)
convlayer = Conv1D(filters=filternumber, kernel_size=3, activation='relu', padding='same')(convlayer)
# convlayer = Flatten()(convlayer)
# embedded_sequences = Flatten()(embedded_sequences)
convlayer = keras.layers.concatenate([embedded_sequences, convlayer])
for i in range(0, 5):
    conv_p = MaxPooling1D(pool_size=3, strides=2)(convlayer)
    convlayer = Conv1D(filters=filternumber, kernel_size=2, activation='relu', padding='same')(conv_p)
    convlayer = Conv1D(filters=filternumber, kernel_size=2, activation='relu', padding='same')(convlayer)
    convlayer = keras.layers.concatenate([convlayer, conv_p])
merged = MaxPooling1D(pool_size=3, strides=2)(convlayer)
merged = Flatten()(merged)
merged = Dense(600, activation=act)(merged)
...
STAMP = 'dpcnn'+str(loadvector)+'_vectors_%.2f_%.2f' % (filternumber,rate_drop_dense)
print(STAMP)

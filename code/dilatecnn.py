...
for (ft_sz,dile) in zip(filter_sizes,dilation):
     conv = Conv1D(filternumber, kernel_size=ft_sz, padding='valid', activation='tanh')(embedded_sequences)
     #conv = Dropout(rate_drop_dense)(conv)
     #conv = BatchNormalization()(conv)
     # conv = MaxPooling1D(pool_size=dile, strides=1, padding='valid')(conv)
...
STAMP = 'dilatecnn'+str(loadvector)+'_vectors_%.2f_%.2f' % (filternumber,rate_drop_dense)
print(STAMP)

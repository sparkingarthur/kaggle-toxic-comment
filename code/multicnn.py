...
for (ft_sz,dile) in zip(filter_sizes,dilation):
     ...
     #conv = keras.layers.multiply([conv, f_W])
     convs.append(conv)
...
STAMP = 'multicnn'+str(loadvector)+'_vectors_%.2f_%.2f' % (filternumber,rate_drop_dense)
print(STAMP)
...
y_test = model.predict([test_data], batch_size=256, verbose=1)

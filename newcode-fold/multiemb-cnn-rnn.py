...
for word, i in word_index.items():
    ...
    if embedding_vector_fasttext is not None:
        ...
        if embedding_vector_word2vec is not None:
            embedding_matrix_fasttext[i] = embedding_vector_word2vec
...
def buildmodel():
    ...
    for (ft_sz, dile) in zip(filter_sizes, dilation):
        x = Conv1D(filternumber, kernel_size=ft_sz, padding='same')(embedded_sequences_glove)
        x1 = PReLU()(x)
        x2 = Activation('tanh')(x)
        x3 = Activation('linear')(x)
        # x = concatenate([x1,x2])
        #x1 = GlobalMaxPool1D()(x)
        #x2 = AttentionWeightedAverage()(x)
        x = concatenate([x1,x2,x3])
        y = Conv1D(filternumber, kernel_size=ft_sz, padding='same')(embedded_sequences_fasttext)
        y1 = PReLU()(y)
        y2 = Activation('tanh')(y)
        y3 = Activation('linear')(y)
        #y = Activation('tanh')(y)
        #y1 = GlobalMaxPool1D()(y)
        #y2 = AttentionWeightedAverage()(y)
        y = concatenate([y1, y2, y3])

        # y2 = Activation('tanh')(y)
        # y = concatenate([y1,y2])
        conv = concatenate([x,y])
        #conv = Dropout(0.1)(conv)
        convs.append(conv)

    merged = keras.layers.concatenate(convs)
    merged_l = Bidirectional(CuDNNLSTM(64))(merged)
    # merged_l = Activation('relu')(merged_l)
    merged_r = Bidirectional(CuDNNLSTM(64, go_backwards=True))(merged)
    # merged_r = Activation('relu')(merged_r)
    merged = keras.layers.concatenate([merged_l, merged_r])
...
for i in range(Kfold):
    ...
    if len(r) == 0:
        ...
        r += cv_models[i].predict(test_data,batch_size=512)
r /= Kfold
index = 'multiemb-cnn-att-10-fold-0'

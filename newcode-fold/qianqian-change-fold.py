...
TRAIN_DATA_FILE = path + 'train_pre.csv'
TEST_DATA_FILE = path + 'test_pre.csv'
...
def buildmodel():
    ...
    # x = Bidirectional(GRU(num_lstm, dropout=rate_drop_lstm, recurrent_dropout=rate_drop_lstm, return_sequences=True))(
    #     embedded_sequences)
    embedded_sequences = SpatialDropout1D(0.15)(embedded_sequences)
    x = Bidirectional(CuDNNLSTM(num_lstm,return_sequences=True))(embedded_sequences)
    # x = Bidirectional(GRU(num_lstm, dropout=rate_drop_lstm, recurrent_dropout=rate_drop_lstm, return_sequences=True))(
    #     x)
    x = Bidirectional(CuDNNGRU(num_lstm, return_sequences=True))(embedded_sequences)
    x = Dropout(rate_drop_dense)(x)
    merged = Attention(MAX_SEQUENCE_LENGTH)(x)
    # merged = keras.layers.concatenate([conv, x])
...
for i in range(Kfold):
    ...
    if len(r) == 0:
        ...
        r += cv_models[i].predict(test_data,batch_size=512)
r /= Kfold
index = 'qianqianchange-'+str(loadvector)+'-10-fold-0'

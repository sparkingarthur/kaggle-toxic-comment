...
def buildmodel():
    ...
    embedded_sequences_glove = Activation('relu')(embedded_sequences_glove)
    ...
    for (ft_sz, dile) in zip(filter_sizes, dilation):
        x = Conv1D(filternumber, kernel_size=ft_sz, padding='same',activation='tanh')(embedded_sequences_glove)
        x = PReLU()(x)
        ...
        x = concatenate([x1,x2])
        y = Conv1D(filternumber, kernel_size=ft_sz, padding='same',activation='tanh')(embedded_sequences_fasttext)
        y = Activation('tanh')(y)
...
for i in range(0,Kfold):
    ...
    for epoch in range(1000):
        model.fit(data_train,labels_train,batch_size=768, epochs=1, verbose=1)
        r = model.predict(data_val ,batch_size=1024)
...
for i in range(Kfold):
    ...
    if len(r) == 0:
        r = cv_models[i].predict(test_data,batch_size=2048)
    else:
        r += cv_models[i].predict(test_data,batch_size=2048)
r /= Kfold
index = 'multiemb-att-cnn-10-fold-0'

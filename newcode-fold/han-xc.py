...
def buildmodel():
    ...
    l_lstm = Bidirectional(CuDNNGRU(32, return_sequences=True))(embedded_sequences)
    ...
    l_lstm_sent = Bidirectional(CuDNNGRU(32, return_sequences=False))(review_encoder)
...
for i in range(0,Kfold):
    ...
    for epoch in range(1000):
        model.fit(data_train,labels_train,batch_size=128, epochs=1, verbose=1)
        r = model.predict(data_val ,batch_size=128)
...
for i in range(Kfold):
    ...
    if len(r) == 0:
        r = cv_models[i].predict(test_data,batch_size=128)
    else:
        r += cv_models[i].predict(test_data,batch_size=128)
r /= Kfold
index = 'hanxc-'+str(loadvector)+'-10-fold-0'

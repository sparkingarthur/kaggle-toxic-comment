...
def buildmodel():
    ...
    merged1 = Dropout(0.1)(merged)

    y = SpatialDropout1D(0.2)(embedded_sequences_glove)
    y = Bidirectional(CuDNNGRU(128, return_sequences=True))(y)
    y = Bidirectional(CuDNNGRU(128, return_sequences=True))(y)
    y = Dropout(0.3)(y)
    y = Conv1D(64, kernel_size=2, padding="same", kernel_initializer="he_uniform")(y)
    avg_pooly = GlobalAveragePooling1D()(y)
    max_pooly = GlobalMaxPooling1D()(y)
    merged2 = concatenate([avg_pooly, max_pooly])
    merged2 = Dropout(0.1)(merged2)
...
for i in range(Kfold):
    ...
    if len(r) == 0:
        ...
        r += cv_models[i].predict(test_data,batch_size=512)
r /= Kfold
index = 'multiemb-pooled-rnncnn-fold'
